...
{  
	try
	{
		...
		if(firstRun) std::cout << devices[0].getInfo<CL_DEVICE_NAME>() << std::endl;
		...
		std::vector<float> vectors(num_particles*2, 0);
			...
			for (size_t i = 0; i < vectors.size(); i += 2)
			{
				...
				double ct = cos(t);
				double v = 0.5;
				vectors[i + 0] = -ct * v; // (-0.5, 0.5)
				vectors[i + 1] = st * v; // (-0.5, 0.5)
			}
		...
		if(!isUniform)
		{
			...
			// pontok elhelyezése lekerekített sarkú négyzetben
			for (size_t i = 0; i < vectors.size(); i+=2)
			{
				double t = i / double(vectors.size() / 2) * (2 * M_PI);
				double st = sin(t); // (-1,1)
...
void CMyApp::Update()
{
	...
	kernel_update.setArg(3, delta_time); // delta time
	kernel_update.setArg(4, G); // gravitational constant
...
void CMyApp::Render()
{
	...
	// újrapareméterezős panel elemei
	ImGui::SetNextWindowPos(ImVec2(300, 400), ImGuiSetCond_FirstUseEver);
	ImGui::Begin("Settings");
	ImGui::Text("Delta time");
	ImGui::SliderFloat("ms", &delta_time, -0.05, 0.05f);
	ImGui::Text("Gravitational constant");
	ImGui::SliderFloat("g", &G, -0.1f, 0.1f);
	ImGui::Text("Number of particles");
	ImGui::SliderInt("pcs", &num_particles, 1, 25000);
	ImGui::Checkbox("Uniform distribution", &isUniform);

	// szimuláció újraindítása a gomb megnyomásakor
